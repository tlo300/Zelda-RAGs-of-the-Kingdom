"""
convert_llm_llama.py
Converts Llama 3.2 1B or 3B Instruct from HuggingFace to a Core ML .mlpackage
with 4-bit palettization quantization, for on-device inference on iOS (HyruleSage).
...
Environment variables:
  ...
  HF_TOKEN       HuggingFace token — required; Llama 3.2 is gated on HuggingFace
                 (accept Meta's licence at huggingface.co/meta-llama before use)

Usage:
    python model/convert_llm_llama.py [--output-dir <dir>]
...
_MODELCONFIG_PATH = Path(__file__).parent.parent / "ios" / "HyruleSage" / "Services" / "ModelConfig.swift"
...
_SIZE_LIMITS = {
    "1B": 800 * 1024 * 1024,           # 800 MB  (Llama 3.2 1B at 4-bit ~600 MB)
...
# Jetsam OOM kill that occurred during MLModel.load() with RangeDim-based designs.
MAX_KV_LEN = 512
...
def _parse_modelconfig_filename(swift_path: Path, variant: str) -> str:
    ...
    in build-ipa-llama.yml), so ModelConfig.swift uses the .mlmodelc extension even
    though this script outputs a .mlpackage.  _assert_filename_sync compares stems only.
    """
    swift_key = "llamaSmall" if variant == "1B" else "llamaLarge"
    try:
...
def _assert_filename_sync(derived: str, from_config: str) -> None:
    ...
    base name (e.g. 'LlamaModel-1B') so build-ipa-llama.yml can compile the right artifact.
    """
...
def convert(output_dir: str, variant: str, hf_token: str) -> None:
    ...
    # Save tokenizer.json alongside the model so build-ipa-llama.yml can bundle it in sync.
    ...
    # which eliminates the Jetsam OOM that killed RangeDim-based designs.
    ...
    # apply_chat_template uses the Llama 3.2 Instruct template from tokenizer_config.json.
...
def main() -> None:
    ...
    if not hf_token:
        ...
        print("       Llama 3.2 is gated — accept Meta's licence at huggingface.co/meta-llama first.", file=sys.stderr)
        sys.exit(1)
