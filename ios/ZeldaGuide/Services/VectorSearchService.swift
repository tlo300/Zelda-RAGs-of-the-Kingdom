...
actor VectorSearchService {
    ...
    /// Uses `ModelConfig.embeddingDimensions` — never a hardcoded dimension.
    ...
    private func vectorSearch(query: [Float], topK: Int) async throws -> [KnowledgeChunk] {
        // CTE materialises the vec0 KNN result so it can be joined with the chunks table.
        let sql = """
            WITH knn AS (
    ...
    private func decodeVectorRow(_ row: [String: any Sendable]) -> KnowledgeChunk? {
        ...
        // For unit-normalised embeddings, L2 distance d and cosine similarity s relate by:
        // s = 1 − d²/2  (from ||a−b||² = 2 − 2·cos θ for unit vectors)
